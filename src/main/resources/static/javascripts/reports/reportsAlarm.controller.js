define([ 'require', 'rchat.module', 'reports/reports.service', 'accounts/accounts.service', 'groups/groups.service'],function(require, rchat) {
	...
	rchat.controller('reportsAlarmCtrl', [ '$scope', '$rootScope', '$http', '$state', '$stateParams', 'reportsService', 'accountsService', 'groupsService'
		, reportsAlarmCtrl ]);

	function reportsAlarmCtrl($scope, $rootScope, $http, $state, $stateParams, reportsService, accountsService, groupsService) {
		var actions = {
			'reports.alarm' : alarm
		};
		 ...
		 * 已过期账号
		 */
		function alarm(agentName,groupName,fullValue,page,size,sort){
			var uId; 
			...
			reportsService.expiringTalkbackUserViews(params, page, size, sort).then(function(data) {
				var alarmUsers = data.content;
				$scope.alarmUsers = alarmUsers;
				var pageInfo = data;
		 ...
		 * 搜索报表
		 */
		$scope.searchAlarm = function(agentName,groupName,fullValue,pageInfo,showPage){
			var page = showPage;
			if(page < 0){
				page = 0;
			...
			alarm(agentName,groupName,fullValue,page,pageInfo.size,null);
		}
		...
		$scope.searchAllAlarm = function() {
			...
			alarm(null,null,null,page,size,sort)
		}
